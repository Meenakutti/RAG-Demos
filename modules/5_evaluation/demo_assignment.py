...
# Load tickets from project data directory
DATA_FILE = Path(__file__).resolve().parents[2] / 'data' / 'synthetic_tickets.json'
with DATA_FILE.open('r', encoding='utf-8') as f:
    tickets = json.load(f)
print(f"✓ Loaded {len(tickets)} support tickets")

# Load evaluation queries with ground truth (path relative to this file)
EVAL_FILE = Path(__file__).resolve().parent / 'evaluation_queries.json'
